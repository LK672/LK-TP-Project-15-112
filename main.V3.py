...
def redrawAll(app):
    ...
    for fireballSpell in app.fireball:
        if fireballSpell.status == True:
            fireballSpell.drawProjectile(app.width, app.height, app.img)
    
    for dummy in app.dummy:
        if dummy.status == True and app.nodeX == 0 and app.nodeY == 0:
            dummy.spawnEnemy()
            dummy.displayHealth()
            for fireball in app.fireball:
                dummy.checkProjectile(fireball)
                dummy.checkStatus()
    
    # if app.dummy.status == True and app.nodeX == 0 and app.nodeY == 0:
    #     app.dummy.spawnEnemy()
    #     app.dummy.displayHealth()
    #     for fireball in app.fireball:
    #         app.dummy.checkProjectile(fireball)
    #         app.dummy.checkStatus()
...
def onMousePress(app, mouseX, mouseY):
    ...
    # if app.introCinematic == True:
        ...
        app.fireball.append(newFireball)
...
def onKeyPress(app, key):
    if key == 'd': 
        ...
        newDummy = Enemy('testDummy', 'Test Dummy.png', 512, 
                          random.randrange(0,app.width), random.randrange(0,app.height), [], True, None)
        app.dummy.append(newDummy)
