...
def onAppStart(app):
    ...
    app.enemyIceShard = CMUImage(Image.open('Enemy Ice Shard.png'))
    app.iceSpiritEnemy = CMUImage(Image.open('Ice Spirit.png'))
    app.iceSpiritProjectiles = []

    app.SpellBookIndexes = set()
    app.spellsList = []
    app.tryItem = True
...
def redrawAll(app):
    ...
    for projectile in app.iceSpiritProjectiles:
        if projectile.status == True:
            projectile.launch()
            projectile.checkInteract(projectile.damage)
                ...
                if enemy.stats[0] == 1:
                    ...
                    if enemy.stats[3] % 60 == 0:
                        ...
                        if enemy.interact() == True:
                            wizard.takeDamage(enemy.stats[2])
                elif enemy.stats[0] == 3:
                        enemy.enemyAlg3(wizard.centerX, wizard.centerY, 
                                        app.width, app.height, 
                                        EnemyProjectile(app.enemyIceShard, enemy.centerX, enemy.centerY, 
                                                                        wizard.centerX, wizard.centerY, 
                                                                        app.width, app.height, 100, True))
                        
                    # app.iceSpiritProjectiles[-1].launch()
                    # app.iceSpiritProjectiles[-1].checkInteract()
                for spell in app.spells:
...
def onMousePress(app, mouseX, mouseY):
    ...
    if app.introCinematic == False and app.thirdPrompt == None and app.items == []:
        app.items = [Item(100, 'Lightning Essence', 'Lightning Essence.png', app.width, app.height)]

        app.spellBooks = [SpellBook("A Basic Spell Only Requiring A Little Ingenuity.", 
                                        'fireball', [(None, 0)], False, False, 
                                        Projectile('fireballI', 'Fireball I.png', 50, 
                                        wizard.centerX, wizard.centerY, [20], True,
                                        app.fireball)),
                            SpellBook("Harness The Power Of Thunder With 1 Essence.", 
        ...
        app.spellsList = ['fireballI', 'Lightning Serpent']
                ...
                for i in range(len(app.spellBooksCollected[app.spellBookSelected[1]].reqMaterials)):
                    ...
                    if currMaterial in app.inventory:
                        if app.inventory[currMaterial] > numMaterial:
                            ...
                            app.learnSpell = True
                            app.SpellBookIndexes.add(app.spellsList.index(app.spellBooksCollected[app.spellBookSelected[1]].projectile.name))
...
def onStep(app):
    ...
    if (app.thirdPrompt == None and app.introCinematic == False and app.drawItem == False
        and (app.nodeX != 0 or app.nodeY != 0) and app.tryItem == True 
    ...
    if (app.thirdPrompt == None and app.introCinematic == False and app.drawSpellbook == False
        and app.nodeX == 0 and app.nodeY == 0 and app.roundNum >= 2 and app.survival == False):
            ...
            if app.spellBook != None and currSpellBookIndex not in app.SpellBookIndexes:
    ...
    if app.totalCounter <= 0:
        if app.survival == True:
            ...
            app.timerColor = 'red'
            app.totalCounter = (60 * 20)
    if app.survival == True:
        ranNum = random.randrange(0, 1000)
        if ranNum >= (995 - (app.roundNum * 3)) and app.roundNum >= 3:
        ...
        elif ranNum >= 990 and app.roundNum >= 4:
            newEnemy = Enemy('Ice Spirit', app.iceSpiritEnemy, 128,
                             random.randrange(0, app.width), random.randrange(0, app.height),
                             [3, 2, 300], True)
            app.iceSpiritProjectiles.append(EnemyProjectile(app.enemyIceShard, newEnemy.centerX, newEnemy.centerY, 
                                                                        wizard.centerX, wizard.centerY, 
                                                                        app.width, app.height, 300, True))
            app.enemies.append(newEnemy)
        elif ranNum >= (985 - (app.roundNum * 5)) and app.roundNum >= 2:
    ...
    if app.moveLeft == True:
        if wizard.centerX <= 0: 
            app.nodeX -= 1
            app.drawItem = False
            wizard.centerX = app.width - 20
    if app.moveUp == True:
        if wizard.centerY <= 0:
            app.nodeY += 1
            app.drawItem = False
            wizard.centerY = app.height - 20
    if app.moveDown == True:
        if wizard.centerY >= app.height:
            app.nodeY -= 1
            app.drawItem = False
            wizard.centerY = 20
    if (wizard.centerX >= app.width or wizard.centerX <= 0 or 
        wizard.centerY <= 0 or wizard.centerY >= app.height):
            app.tryItem = True
    app.count += 1
    ...
    for j in range(len(app.spells)):
        if j >= len(app.spells):
            ...
            app.spells.pop(j)
    iceList = []
    for iceIndex in range(len(app.enemies)):
        if app.enemies[iceIndex].name == 'Ice Spirit':
            iceList.append(app.enemies[iceIndex])
    for k in range(len(iceList)):
        if app.iceSpiritProjectiles[k].status == False:
            app.iceSpiritProjectiles.pop(k)
            app.iceSpiritProjectiles.insert(k, EnemyProjectile(app.enemyIceShard, iceList[k].centerX, iceList[k].centerY, 
                                                                        wizard.centerX, wizard.centerY, 
                                                                        app.width, app.height, 100, True))
    if app.moveRight == True:
...
class Enemy:
    ...
    def spawnEnemy(self):
        if type(self.img) != 'str':
            drawImage(self.img, self.centerX, self.centerY, align = 'center')
        else:
    ...
    def generateBFSListGreater(self, targetX, targetY, dimX, dimY):
        graph = []
        for row in range((dimY // 25)):
            newRow = []
            for col in range((dimX // 25)):
                if distance(self.centerX, self.centerY, col * 25, row * 25) <= 40:
                    newRow.append((row, col))
                    indexRow, indexCol = row, col
                elif distance(targetX, targetY, col * 25, row * 25) <= 40:
                    newRow.append((row, col))
                    targetRow, targetCol = row, col
                else:
                    newRow.append((row, col))
            graph.append(newRow)
        return (graph, indexRow, indexCol, targetRow, targetCol)
    ...
    def flee(self, wcenterX, wcenterY, currX, currY, di, dimX, dimY):
        dx = random.choice([-1, 0, 1])
        dy = random.choice([-1, 0, 1])
        newX, newY = currX + (dx * 15), currY + (dy * 15)
        if 0 <= newX <= dimX and 0 <= newY <= dimY and distance(wcenterX, wcenterY, newX, newY) >= di:
            return newX, newY
        else:
            x = random.randrange(1, dimX)
            y = random.randrange(1, dimY)
        return x, y
                    
    
    def enemyAlg3(self, wcenterX, wcenterY, dimX, dimY, projectile):
        initialDistance = distance(wcenterX, wcenterY, self.centerX, self.centerY)
        if initialDistance <= 200:
            res = self.flee(wcenterX, wcenterY, self.centerX, self.centerY, initialDistance, dimX, dimY)
            self.centerX, self.centerY = res[0], res[1]
        # else:
        #     projectile.launch
        #     projectile.checkInteract(projectile.damage)
...
class EnemyProjectile:
    ...
    def __init__(self, img, startX, startY, wcenterX, wcenterY, dimX, dimY, damage, status):
        self.img = img
        self.centerX = startX
        self.centerY = startY
        self.wcenterX = wcenterX
        self.wcenterY = wcenterY
        self.dimX, self.dimY = dimX, dimY
        self.damage = damage
        self.status = status
        tempdx = self.wcenterX - startX
        tempdy = self.wcenterY - startY
        magnitude = ((tempdx ** 2) + (tempdy ** 2)) ** 0.5
        epic = tempdx / magnitude
        # print(magnitude, epic)
        self.dx = tempdx / magnitude
        self.dy = tempdy / magnitude
    
    def track(self, wcenterX, wcenterY, currX, currY, di, dimX, dimY):
        newX, newY = currX + (self.dx * 17), currY + (self.dy * 17)
        return newX, newY
    
    def launch(self):
        x = wizard.centerX, wizard.centerY
        y = abs(self.centerX - wizard.centerX), abs(self.centerY - wizard.centerY)
        o = self.centerX, self.centerY
        ang = math.degrees(math.atan2(x[1]-y[1], x[0]-y[0]) - math.atan2(o[1]-y[1], o[0]-y[0]))
        if ang < 0:
            ang += 360
        drawImage(self.img, self.centerX, self.centerY, align = 'center', rotateAngle = int(ang))
        di = distance(self.centerX, self.centerY, wizard.centerX, wizard.centerY)
        res = self.track(wizard.centerX, wizard.centerY, self.centerX, self.centerY, di, self.dimX, self.dimY)
        self.centerX, self.centerY = res[0], res[1]

    def checkInteract(self, damage):
        if distance(wizard.centerX, wizard.centerY, self.centerX, self.centerY) <= 64:
            if wizard.health - damage <= 2:
                wizard.health = 2
                wizard.status = False
            else:
                wizard.health -= damage
                wizard.InvTimer = 0.5 * 20
            self.status = False
            print('super not cool')
        if self.centerX <= 0 or self.centerX >= self.dimX or self.centerY <= 0 or self.centerY >= self.dimY:
            self.status = False
